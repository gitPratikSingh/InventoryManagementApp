	...
	$.fn.mDatatable = function(options) {
		...
		var Plugin = {
			...
			setupDOM: function(table) {
				...
				$(table).find('tr > th').each(function(i, td) {
					if ($(td).find('span').length === 0) {
						$(td).wrapInner($('<span/>').css('width', Plugin.offset));
						if(i>0){
							var colname = $(td).attr("data-field");
							$(td).append( "<input type='text' id=input_"+ colname + ">");
						}
					}
				});

				$(table).find('tr > td').each(function(i, td) {
					if ($(td).find('span').length === 0) {
						$(td).wrapInner($('<span/>').css('width', Plugin.offset));
					}
				});
			...
			setHeadTitle: function(tablePart) {
				...
				$.each(columns, function(i, column) {				
					...
					if (typeof column['title'] !== 'undefined') {
						...
						// added for per column filter
						var id = column.field;
						//$(th).append( "<input type='text' id=input_"+ id + ">");
					}
			...
			columnHide: function() {
				...
				//console.log("Disabled: Column Preferences- "+meta.columnpreferences);
				var columnpreferences = JSON.parse(meta.columnpreferences);
				...
				$.each(options.columns, function(i, column) {
						...
						if(columnpreferences[field] == false){
							$(tds).hide();
				...
				var sortObj = {
					init: function() {
						if (options.sortable) {
							...
							//$("#input_make_name").click(function(e));
						}
					},
					setIcon: function() {
					...
					sortClick: function(e) {
						// disable sorting on clicking per column filters
